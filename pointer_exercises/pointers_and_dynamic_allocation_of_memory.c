...
/*
    Example of multidimensional arrays (3D, or more)
*/
...
int main()
{
    ...
    // First we malloc space for the array itself
    space = malloc(X_DIM * Y_DIM * Z_DIM * sizeof(char));

    //Now we will allocate space for an array of pointers, which will eventually point to the starting addresses of 2D arrays (these 2D arrays are pointers to pointers)
    Arr3D = malloc(Z_DIM * sizeof(char **));

    // To each pointer created above we will assign a pointer to a newly allocated array of pointers to row
    for(z = 0; z < Z_DIM; z++)
    {
        ...
        for(y = 0; y < Y_DIM; y++)
        {
            Arr3D[z][y] = space + (z*(X_DIM * Y_DIM) + y*X_DIM);    //For each space in this array we put a pointer to the first element of each row in the array space originally allocated
        }
    }

    // Now for each address in our 3D array we will check if the indexing of Arr3D is done continuosly
    for(z = 0; z < Z_DIM; z++)
    {
        printf("Location of array %d is %p\n", z, *Arr3D[z]);
        for(y = 0; y < Y_DIM; y++)
        {
            printf("  Array %d and Row %d starts ad %p\n", z, y, Arr3D[z][y]);
